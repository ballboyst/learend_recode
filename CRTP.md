...
graph TD;
50([ドメイン管理セッションの識別])
-->51[LO5で作成したdcorp-ciセッションに移動]
-->52[dcorp-ciでプロセスにマイグレーション]
-->53[新しいセッション開始]
-->54[プロセスインジェクションしLACheckを実行]
55[dcorp-ciでリスナーを設定後、インプラント生成]
-->56[sa-sc-enumでサービス列挙]
-->57[remote-sc-stopでサービス停止]
-->58[BinLoaderアアップロード]
-->59[scshellでサービスのパスを変更しインプラントをメモリ上で実行]
-->5a[dcorp-mgmtのセッション開始]
-->5b[dcorp-mgmtで実行中のプロセスを確認]
-->5c[dcorp-mtmgでプロセスにマイグレーション]
-->5d[新しく貼られたセッションに移行]
-->|クレデンシャルボールトの使用も可能|5e[execute-assemblyで変換mimikatzをプロセスインジェクションし実行]
-->5f[クレデンシャル入手]
-->5g[dcorp-stdに戻り、プロセスインジェクションでRubeusを実行し入手したクレデンシャルを使ってticketをインポート]
-->5h[プロセスインジェクションでLACheckを使いdcorp¥svcadminとしてローカル管管理へのアクセスを見つける]
-->5i[プロセスインジェクションでRubeusを実行しticketを消去する]
5g-->5j[lsコマンドでDCの中を確認]

5k[LO5から]-->5l[dcorp-adminsrvセッションに移動]-->5e
5y([dcorp-ciを使用したドメイン管理者権限取得])-->55
5z([派生菅理者を使用したドメイン管理者権限取得])-->5k
```

## LO8
```mermaid
graph TD;
81[Rubeusを使いdcorp-std¥svcadminの資格情報でチケットをインポート]
-->82[lsコマンドでdcorp-dcのフォルダ内を確認]
-->83[sa-sc-enumでリモートサービス列挙]
-->84[ピボットリスナーを設定後、インプラント生成]
-->85[インプラントをターゲットにアップロード]
-->86[簡易サーバーを起動し待ち受け]
-->87[scshellでインプラントを実行]
-->88[dcorp-dcセッションが開始]
-->89[dcorp-dcセッションに移動]
-->8a[プロセスにマイグレーションし新しいセッションを開始]
-->8b[新しいセッションに移動]
-->8c[プロセスインジェクションでmimikatzを実行]
-->8d[ログオンパスワードがダンプされる]

8e[dcorp-stdに移動]
-->8f[DCSync引数を含むmimikatzを.NETバイナリに変換]
-->8g[dcorp-dcセッションに移動し、プロセスインジェクションで生成した.NETバイナリを使用]
-->8h[krbtgtのNTLMが判明]
-->8i[Rubeusでゴールデンチケットをインポート]
-->8j[チケット一覧を表示しdcorp-dcへのドメイン管理者アクセス権を確認]
-->8k[dcorp-dcのフォルダ内を確認]
-->8l[チケットの削除]
8e-->8m[dcorp-stdセッションでDCSyncを実行]
8m-->8h
```
